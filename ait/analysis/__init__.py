    ...
    if mutated_only:
        findings.append(
            Finding(
                ...
                remediation_note="Inspect state-dependent branches and scope checks in the integration.",
            )
        )
        ...
        if sensitive_hit:
            findings.append(
                Finding(
                    ...
                    evidence=f"Response from {exchange.path} contained a configured sensitive marker.",
                    expected_behavior="Sensitive marker fields should not be retrieved unless explicitly approved.",
                    observed_behavior=f"Observed fields: {', '.join(exchange.extracted_fields) or 'unknown'}",
                    remediation_note="Restrict the integration to minimum necessary fields and revalidate scopes.",
                )
            )

    # Advanced detectors
    findings.extend(detect_scope_violations(target, exchanges))
    findings.extend(detect_anomalies(exchanges))
    findings.extend(detect_rate_limit_violations(target, exchanges))
