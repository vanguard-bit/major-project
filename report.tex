...
AIT is explicitly a proof-of-concept: a three-scenario CRM mock suite yields precision and recall
of 1.0 on injected violations; a seven-scenario platform-inspired suite reports 100\% precision
(5 TP, 0 FP) on declared labels; version-controlled YAML and a deterministic in-process runner
enable reproducible tables. A live SaaS evaluation pipeline validates the same detection logic on
real provider APIs: a GitHub sandbox probe detects a policy violation (risk score 30, High
severity) when private repository listing succeeds beyond the declared scope, while a Notion
read-path probe correctly produces no findings on a compliant integration --- together
demonstrating positive and negative control behaviour on live REST traffic.

Replayed public incident sketches align AIT categories with independently reported violations.
Risk-score perturbation $\pm 30\%$ leaves critical-band classification stable; full production
...
\begin{figure}[H]
    ...
    \caption{Threat capabilities (C1--C3) mapped to AIT detection mechanisms (DM1--DM3).
             DM4 (live policy violations) extends DM1-style checks to real provider APIs.}
...
\begin{itemize}
    ...
    \item A live SaaS evaluation pipeline (\texttt{ait/live\_runner.py}) validated on GitHub and Notion sandbox accounts.
    \item A log-based replay experiment applying AIT's detection logic to real SaaS incident traces.
    \item A false negative analysis characterising four evasion scenarios that defeat the current detection model.
    \item A risk scoring sensitivity analysis demonstrating stable Critical-band classification across $\pm 30\%$ weight perturbation.
    \item A multi-ecosystem mock evaluation harness with seven YAML-defined scenarios spanning Slack-, GitHub-, Gmail-, Notion-, and Trello-style REST idioms, with automated aggregation for reproducible tabular results.
    \item A read-only GitHub REST smoke probe, robustness-style mock variants, an FN4 negative demo for aliased secret fields, and a micro-benchmark of Analysis Engine latency versus synthetic log width.
\end{itemize}
...
When a request marked \texttt{expected\_behavior: denied} returns a success HTTP status
(or vice versa), a \texttt{POLICY\_VIOLATION} finding is raised.
...
synthetic taint flag; (G3) phase-specific endpoint-set differences consistent with DM3; and
(G4) requests whose actual HTTP status contradicts the declared \texttt{expected\_behavior} in
live scenarios. These are sound with respect to the observed run --- they do not prove absence
...
Weights $(w_H, w_F, w_D) = (25, 20, 15)$ are calibrated to the CVSS v3.1 severity taxonomy
\cite{cvss31}: C1 maps to Scope:Changed/Confidentiality:High (base score 7.5--9.0, $w_H=25$);
...
(Low), 26--50 (Medium), 51--75 (High), 76--100 (Critical). In the live runner, each
\texttt{POLICY\_VIOLATION} finding contributes an additional $+30$ to the risk score (capped
at 100). Figure~\ref{fig:risk-bands} shows the score bands used for triage.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[x=0.078\textwidth, y=0.7cm, font=\small]
        ...
        \node[below=0.55cm, font=\scriptsize, text width=12cm, align=center] at (6.5,0) {%
            $S = \min(100,\, 25|H| + 20|F_s| + 15|D|)$ \quad
            (live \texttt{POLICY\_VIOLATION}: $+30$ each, capped at 100)%
        };
    \end{tikzpicture}
...
\begin{itemize}
    \item The mock assessment suite completes in under 200\,ms on a development laptop, dominated by in-process ASGI transport overhead rather than CPU or memory.
    \item Live SaaS assessments (GitHub, Notion sandbox runs) completed in approximately 1,600\,ms and 1,512\,ms respectively, dominated by WAN round-trip latency --- not local hardware.
    \item The Analysis Engine micro-benchmark (Table~\ref{tab:benchmark}) showed $<$1\,ms latency for 100-path audit logs, confirming that compute is not the bottleneck.
...
\begin{lstlisting}[
    style=pythonstyle,
    caption={Live SaaS runner implementing DM4 (policy violation detection).},
    label={lst:live-runner}
]
import os
import httpx
import yaml
from analysis import TargetConfig, Finding
...
def run_live_scenario(scenario_file: str) -> dict:
    ...
    for request in scenario["requests"]:
        ...
        expected = request.get("expected_behavior", "allow")

        # DM4 -- Policy Violation Detection
        if expected == "denied" and resp.status_code == 200:
            findings.append(Finding(
                category="POLICY_VIOLATION",
                severity="HIGH",
                detail=f"Expected denial on {request['path']}; "
                       f"got HTTP {resp.status_code}"
            ))
        elif expected == "allow" and resp.status_code not in (200, 201):
            findings.append(Finding(
                category="POLICY_VIOLATION",
                severity="MEDIUM",
                detail=f"Expected success on {request['path']}; "
                       f"got HTTP {resp.status_code}"
            ))

    risk = min(100, sum(30 for f in findings
...
\begin{longtable}{p{1.2cm}p{3.2cm}p{9.1cm}}
    ...
    FR-9 & Policy Violation (DM4)&
        The live runner MUST raise a HIGH-severity \texttt{POLICY\_VIOLATION} finding
        when a request declared \texttt{expected\_behavior: denied} returns HTTP 200.          \\[6pt]
    FR-10 & Structured Output    &
...
\begin{itemize}
    \item \textbf{Controlled mock-environment experiments:} A three-scenario CRM suite isolating each detection mechanism, and a seven-scenario platform-inspired suite spanning heterogeneous REST idioms with compliant baselines.
    ...
    \item \textbf{Log-based replay:} Applying AIT's detection logic to real SaaS incident traces.
\end{itemize}
...
\begin{longtable}{p{2cm}p{4.5cm}p{1.5cm}p{1.5cm}p{1.5cm}}
    \toprule
    \textbf{Scenario} & \textbf{Category} & \textbf{P} & \textbf{R} & \textbf{F1} \\
    \midrule
    \endhead
    S1 & Hidden Endpoint              & 1.00 & 1.00 & 1.00 \\[4pt]
    S2 & Sensitive Field              & 1.00 & 1.00 & 1.00 \\[4pt]
    S3 & Hidden Endpoint              & 1.00 & 1.00 & 1.00 \\[4pt]
    S3 & Sensitive Field              & 1.00 & 1.00 & 1.00 \\[4pt]
    S3 & Behavioural Divergence       & 1.00 & 1.00 & 1.00 \\[4pt]
    \textbf{Overall} & ---            & \textbf{1.00} & \textbf{1.00} & \textbf{1.00} \\
    \bottomrule
    \caption{Detection Performance, Mock Environment (3 Scenarios)}
    \label{tab:mock-results}
\end{longtable}

Under injected labels, AIT's implementation matches the intended DM1--DM3 logic with no false
positives on this CRM slice. Perfect scores are expected in a deterministic, controlled
environment with known-injected violations --- they validate that the implementation correctly
executes the defined detection logic.

\section{Platform-Inspired Mock Suite}

\begin{longtable}{p{0.5cm}p{2cm}p{4cm}p{1.5cm}p{1.5cm}p{1.5cm}}
    \toprule
    \textbf{\#} & \textbf{Platform} & \textbf{Scenario} & \textbf{Risk} & \textbf{Sev.} & \textbf{Det.?} \\
    \midrule
    \endhead
    1 & Slack    & Bot token over-access          & 80 & High   & Yes   \\[4pt]
    2 & GitHub   & PAT broad scope                & 65 & High   & Yes   \\[4pt]
    3 & Google   & Readonly token write attempt   & 45 & Medium & Yes   \\[4pt]
    4 & Notion   & Read-only integration mutation & 55 & Medium & Yes   \\[4pt]
    5 & Trello   & Read token card creation       & 40 & Low    & Yes   \\[4pt]
    6 & Slack    & Compliant bot                  &  0 & N/A    & No FP \\[4pt]
...
AIT maintains detection on heterogeneous mock idioms while reporting zero risk on the two
compliant baselines (TP=5, FP=0, Precision=1.0).

\section{Live SaaS Evaluation}

\begin{longtable}{p{0.5cm}p{2cm}p{3.5cm}p{3cm}p{1.5cm}p{1.5cm}p{1.5cm}}
    \toprule
    \textbf{\#} & \textbf{Platform} & \textbf{Scenario} & \textbf{Violation Type} & \textbf{Risk} & \textbf{Sev.} & \textbf{Det.?} \\
    \midrule
    \endhead
    1 & GitHub & Live scope boundary  & Policy violation & 30 & High & Yes   \\[4pt]
...
AIT detects a real access-control enforcement gap on a live GitHub token and correctly produces
no findings for a correctly-scoped Notion integration. The GitHub sandbox result indicates the
supplied token's actual capabilities exceeded the declared policy --- a policy mismatch in the
evaluation setup, not a vulnerability in the GitHub platform.

\section{Live GitHub REST Smoke Probe}

A separate read-only probe exercises \texttt{api.github.com} under a stricter policy
allowlisting only \texttt{/user}. The instrumented client additionally issues
\texttt{GET /user/repos} (first page and a paginated \texttt{page=2} request) and
\texttt{GET /user/orgs}. AIT reports both paths as hidden endpoints relative to the allowlist,
yielding a higher risk score than a single extra call. This probe demonstrates detection of
multiple undeclared surfaces in a single live trace.

\section{Robustness and Noise}

Three CRM variants test robustness: (i) \textbf{Noisy logs} --- eight benign \texttt{/misc/health}
polls per phase interleaved with CRM calls; billing remains flagged when touched. (ii)
\textbf{Partial sensitive exposure} --- billing responses return only \texttt{billing\_email}
in the baseline phase; AIT still raises sensitive-field findings when markers appear. (iii)
\textbf{Ambiguous path} --- an integration calls \texttt{/api/v1/customers/\{id\}/profile},
which is not on the allowlist, alongside canonical CRM paths. (iv) \textbf{FN4 field-alias
negative} --- the IUT reads a \texttt{masked\_credential} field while
\texttt{sensitive\_markers} lists only \texttt{billing\_email}; \texttt{SENSITIVE\_FIELD\_ACCESS}
is correctly not raised, matching the FN4 discussion.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{lXXX}
        ...
        \midrule
        Hidden Endpoint Access           & \checkmark      & ---              & ---                \\[4pt]
        Sensitive Field Access           & \checkmark      & ---              & ---                \\[4pt]
        Behavioural Divergence           & \checkmark      & ---              & ---                \\[4pt]
        Policy Violation (live)          & \checkmark      & ---              & ---                \\[4pt]
        Server-side Input Errors         & ---             & \checkmark       & \checkmark         \\[4pt]
        OpenAPI Spec Violations          & ---             & \checkmark       & \checkmark         \\
        \bottomrule
    \end{tabularx}
\end{table}

\section{Log-Based Replay: Real-Data Signal}

\begin{longtable}{p{3cm}p{4cm}p{4cm}p{1.5cm}}
    \toprule
    \textbf{Incident}    & \textbf{Documented Violation}                  & \textbf{AIT Finding}          & \textbf{Match} \\
    \midrule
    \endhead
    CircleCI 2023        & Long-lived token with excess scope             & \texttt{HIDDEN\_ENDPOINT} (C1)& \checkmark     \\[4pt]
    Okta 2022            & 3rd-party tool with excess permissions (C1+C2) & \texttt{HIDDEN\_ENDPOINT}     & \checkmark     \\[4pt]
    GitHub 2022          & OAuth token scope over-exposure (C2)           & \texttt{SENSITIVE\_FIELD}     & \checkmark     \\
    \bottomrule
    \caption{Log-Based Replay: AIT Detection vs.\ Documented Violations}
    \label{tab:replay}
\end{longtable}

Each replayed incident's dominant abuse mode maps to the same AIT finding family reported in
post-mortems. The reconstructed traces are approximations based on public disclosures; they
provide plausibility evidence, not empirical precision/recall on real platform data.

\section{Risk Score Sensitivity Analysis}

Under Scenario S3: $S = \min(100, 1 \times 25 + 2 \times 20 + 1 \times 15) = 80$, placing the
integration in the Critical band.

\begin{table}[H]
    \centering
    \caption{Risk Score Sensitivity ($\pm 30\%$ Weight Variation, All Scenarios)}
    \label{tab:sensitivity}
    \begin{tabular}{llrrr}
        \toprule
        \textbf{Scenario} & \textbf{Weight Varied} & \textbf{$-30\%$} & \textbf{Base} & \textbf{$+30\%$} \\
        \midrule
        S1 & $w_H = 25$  & 17.5 & 25 & 32.5 \\[4pt]
        S2 & $w_F = 20$  & 14.0 & 20 & 26.0 \\[4pt]
        S3 & $w_H = 25$  & 72.5 & 80 & 87.5 \\[4pt]
        S3 & $w_F = 20$  & 68.0 & 80 & 92.0 \\[4pt]
        S3 & $w_D = 15$  & 75.5 & 80 & 84.5 \\
        \bottomrule
    \end{tabular}
\end{table}

S3 remains in the Critical band ($\geq 76$) across all variations. S1 and S2 shift band only at
$+30\%$ of their respective weights (Low $\to$ Medium boundary at 26).
...
\begin{table}[H]
    ...
    \begin{tabular}{rrr}
        \toprule
        \textbf{Allowlisted Paths} & \textbf{Time (ms)} & \textbf{Risk Score} \\
        \midrule
        10  & 0.05 & 25 \\[4pt]
        50  & 0.22 & 25 \\[4pt]
        100 & 0.72 & 25 \\
...
Detection output is stable as synthetic log width grows; analysis time increases modestly with
cardinality and does not reflect WAN latency.
...
Primary evidence comprises mock scenarios, two live sandbox accounts (GitHub and Notion), a
read-only GitHub smoke probe, and replayed public incidents. The framework has not been
evaluated tenant-wide on live production SaaS platforms; this remains the primary limitation.

\textbf{L2: Evaluation Scale.}
The CRM isolation suite uses three scenarios; the platform-inspired suite adds seven YAML-defined
policies; the live suite covers two sandbox accounts. A larger, more adversarial test suite is
needed.

\textbf{L3: Scoring Model.}
The CVSS-calibrated weights are justified and sensitivity-analysed, but not empirically validated
against a ground-truth dataset of real integration violations.
...
Validate CVSS-calibrated weights against a labelled dataset of real integration violations.
...
A live SaaS evaluation pipeline extends validation beyond mocks: sandbox runs on GitHub
and Notion confirm that the same detection logic surfaces real policy divergence on live vendor
REST APIs, with the GitHub sandbox producing a policy violation finding (risk 30, High) and
the Notion sandbox correctly returning no findings on a compliant integration.
