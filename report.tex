...
enable reproducible tables. A live SaaS evaluation pipeline
...
\begin{itemize}
    ...
    \item Live SaaS assessments (GitHub, Notion sandbox runs) are \texttt{NOT RUN} (Task~6 blocked without credentials); measured WAN timings are therefore unavailable.
...
\begin{longtable}{p{0.5cm}p{2cm}p{3.5cm}p{3cm}p{1.5cm}p{1.5cm}p{1.5cm}}
    ...
    \midrule
    \endhead
    1 & GitHub & Live scope boundary  & --- & NOT RUN & --- & BLOCKED \\[4pt]
    2 & Notion & Read-path validation & --- & NOT RUN & --- & BLOCKED \\
    \bottomrule
    \caption{Live SaaS Evaluation Results (Sandbox Accounts; Task~6 blocked without credentials)}
...
\texttt{/user/orgs} requests. Empirical smoke-probe results are \texttt{NOT RUN}
(Task~6 blocked). When executed, the signal of interest is policy mismatch relative to
the allowlist, not server-side exploitability.
...
policies; live sandbox plans exist but empirical runs are blocked. A larger, more adversarial test suite is
needed.
