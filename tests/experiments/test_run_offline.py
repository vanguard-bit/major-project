...
def test_offline_cli_ordering_and_success(tmp_path: Path):
    ...
    assert "=== 1/4 scenarios ===" in result.output
    assert "=== 2/4 risk sensitivity ===" in result.output
    assert "=== 3/4 incident replay ===" in result.output
    assert "=== 4/4 benchmark ===" in result.output
    assert result.output.index("=== 1/4") < result.output.index("=== 2/4")
    assert result.output.index("=== 2/4") < result.output.index("=== 3/4")
    assert result.output.index("=== 3/4") < result.output.index("=== 4/4")
    manifest = json.loads(
        (output / "derived" / "offline_manifest.json").read_text(encoding="utf-8")
    )
