...
async def main():
    # Start Mock SaaS and Integration Server
    # Using 'python3 -m uvicorn' to ensure we use the same environment
    saas_proc = subprocess.Popen(
        ["python3", "-m", "uvicorn", "ait.mock_saas:app", "--port", "8001", "--log-level", "error"]
    )
    int_proc = subprocess.Popen(
        ["python3", "-m", "uvicorn", "ait.demo_integration:app", "--port", "8000", "--log-level", "error"]
    )
    ...
    try:
        ...
        for yf in yaml_files:
            ...
            # Severity mapping as described in PLAN.md
            severity = "Low"
            ...
            # Specific labels for Paper Table
            detected = "✅ Yes"
        ...
        with open(results_dir / "precision_calculation.txt", "w") as f:
