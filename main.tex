...
instrumented, allowlisted sandbox probes against real provider APIs, but
GitHub and Notion sandbox Task~6 execution remains \textbf{BLOCKED}
without credentials and is marked \texttt{NOT RUN}---no live risk scores
or timings are claimed here. Replayed public
...
report return ($T=110$\,ms). Live assessments are expected to be
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}clllccc@{}}
    ...
    \midrule
    1 & GitHub & Live scope boundary & --- & NOT RUN & --- & BLOCKED \\
    2 & Notion & Read-path validation & --- & NOT RUN & --- & BLOCKED \\
    \bottomrule
  \end{tabular}
\end{table}
...
\texttt{/user/orgs} requests. Empirical smoke-probe results are
\texttt{NOT RUN} (Task~6 blocked). When executed with a read-only PAT via
\texttt{AIT\_GITHUB\_TOKEN}, the signal of interest is
...
policies; live sandbox plans exist but empirical runs are blocked.
Together they
