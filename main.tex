...
deterministic in-process runner enable reproducible tables. A live SaaS
evaluation pipeline validates the same detection logic on real provider
APIs: a GitHub sandbox probe detects a policy violation (risk score 30,
High severity) when private repository listing succeeds beyond the
declared scope, while a Notion read-path probe correctly produces no
findings on a compliant integration---together demonstrating positive
and negative control behavior on live REST traffic. Replayed public
incident sketches align AIT categories with independently reported
violations. Risk-score perturbation $\pm30\%$ leaves critical-band
classification stable; full production generalization remains future
work.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A \emph{live SaaS evaluation pipeline} (\texttt{ait/live\_runner.py})
        ...
        reconciliation---validated on GitHub and Notion sandbox accounts.
  ...
  \item A \emph{read-only GitHub REST smoke probe} hitting multiple
        undeclared paths (\texttt{/user/repos} with pagination,
        \texttt{/user/orgs}), \emph{robustness-style} mock variants
        (noisy logs, partial sensitive payloads, ambiguous paths), an
        FN4 negative demo for aliased secret fields, and a
        micro-benchmark of Analysis Engine latency versus synthetic
        log width.
\end{enumerate}
...
\textbf{DM4 --- Policy Violation Detection} (live runner):
When a request marked \texttt{expected\_behavior: denied} returns a
success HTTP status (or vice versa), a \texttt{POLICY\_VIOLATION}
finding is raised. This mechanism extends the core three detectors for
live scenarios where expected access-control enforcement can be
...
(or carry the synthetic taint flag); (G3)~phase-specific endpoint-set
differences consistent with Eq.~\eqref{eq:divergence}; and
(G4)~requests whose actual HTTP status contradicts the declared
\texttt{expected\_behavior} in live scenarios.
...
(Medium), 51--75 (High), 76--100 (Critical). In the live runner,
each \texttt{POLICY\_VIOLATION} finding contributes an additional
$+30$ to the risk score (capped at 100), reflecting the empirical
significance of access-control enforcement failures on real APIs.
...
A typical mock assessment completes in under 200\,ms: token resolution
...
report return ($T=110$\,ms). Live assessments are dominated by
WAN latency; the GitHub and Notion sandbox runs completed in
approximately 1,600\,ms and 1,512\,ms respectively (round-trip to
provider endpoints).
...
(2)~a live SaaS evaluation executing explicit sandbox scenarios against
real provider APIs (GitHub and Notion); (3)~a read-only GitHub REST
smoke probe under an explicit allowlist policy; (4)~a tool capability
comparison with RESTler and EvoMaster; and (5)~a log-based replay
experiment applying AIT's detection logic to real SaaS incident traces.
Components~(1)--(3) provide increasing levels of evidence, from fully
controlled conditions to live vendor REST traffic. Component~(5)
provides a first real-data alignment signal. Together they mitigate the
concern that mock-only evaluation lacks any live-surface anchor.
...
evaluation (Section~\ref{sec:livesaas} and future work,
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}cllccc@{}}
    ...
    \midrule
    1 & Slack   & Bot token over-access         & 80 & High    & Yes \\
    2 & GitHub  & PAT broad scope               & 65 & High    & Yes \\
    3 & Google  & Readonly token write attempt  & 45 & Medium  & Yes \\
    4 & Notion  & Read-only integration mutation& 55 & Medium  & Yes \\
    5 & Trello  & Read token card creation      & 40 & Low     & Yes \\
    6 & Slack   & Compliant bot                 & 0  & N/A     & No FP \\
    7 & GitHub  & Compliant app                 & 0  & N/A     & No FP \\
    \bottomrule
  \end{tabular}
\end{table}
...
behavior, we executed AIT's live runner against two sandbox accounts
using read-only personal access tokens stored in environment variables
outside the repository. Scenarios are defined as YAML files in
\texttt{live\_test\_cases/} specifying the provider base URL, declared
scope, sensitive markers, and explicit baseline/mutated request
sequences with expected HTTP status codes and access-control
expectations. Results are written to \texttt{results/live\_saas/}.

\noindent\textbf{GitHub sandbox (\texttt{github\_live.yaml}):}
Declared policy: \texttt{read:user, public\_repo}; expected endpoints:
\texttt{/user} and \texttt{/user/repos}. The mutated phase issues a
\texttt{GET /user/repos?visibility=private} request expected to be
denied (HTTP~403 or~404). The token returned HTTP~200 for both the
public and private listing requests, indicating that the supplied
token's actual scope exceeded the declared policy. AIT raised a
\texttt{POLICY\_VIOLATION} finding with risk score~30 and severity
High, confirming the policy mismatch.

\noindent\textbf{Notion sandbox (\texttt{notion\_live.yaml}):}
Declared policy: \texttt{read content}; expected endpoints:
\texttt{/v1/users/me} and \texttt{/v1/search}. Both baseline and
mutated phases completed successfully with HTTP~200 on expected
endpoints; no undeclared endpoints were reached and no sensitive markers
were observed. AIT reported risk score~0 with no findings---confirming
the integration is compliant with the declared policy.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}clllccc@{}}
    \toprule
    \textbf{\#} & \textbf{Platform} & \textbf{Scenario} & \textbf{Violation Type} & \textbf{Risk} & \textbf{Sev.} & \textbf{Det.?} \\
    \midrule
    1 & GitHub & Live scope boundary & Policy violation & 30 & High & Yes \\
    2 & Notion & Read-path validation & None & 0  & ---  & No FP \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent\textit{Takeaway:} AIT detects a real access-control
enforcement gap on a live GitHub token and correctly produces no
findings for a correctly-scoped Notion integration. These results
demonstrate both positive and negative control behavior on real SaaS
APIs, strengthening external validity beyond mock environments.

\noindent\textit{Interpretation note:} A positive result on the GitHub
sandbox indicates the supplied token's actual capabilities exceeded
the declared policy---a policy mismatch in the evaluation setup, not
a vulnerability in the GitHub platform. The signal of interest is
\emph{policy divergence}, not server-side exploitability. These results
reflect two sandbox accounts under specific token configurations and
are not a measurement of GitHub's or Notion's global OAuth enforcement.
Duration for the GitHub run was approximately 1,603\,ms and for Notion
approximately 1,512\,ms, dominated by WAN round-trip latency.

\subsection{Live GitHub REST Smoke Probe (Optional)}
\label{sec:livegithub}

Complementing the structured sandbox scenarios in
Section~\ref{sec:livesaas}, a separate read-only probe exercises
\texttt{api.github.com} under a stricter policy allowlisting only
\texttt{/user}. The instrumented client additionally issues
\texttt{GET /user/repos} (first page and a paginated \texttt{page=2}
request) and \texttt{GET /user/orgs}. AIT reports both paths as
hidden endpoints relative to the allowlist, yielding a higher risk
score than a single extra call. HTTP status may be 200 or 403
depending on token scopes; the signal of interest is
\emph{policy mismatch}, not server-side exploitability. This probe
demonstrates detection of \emph{multiple} undeclared surfaces in a
single live trace using the same analysis path as mocks, and is
reproducible with any read-only PAT via the \texttt{AIT\_GITHUB\_TOKEN}
...
by network I/O and OAuth/token work, as confirmed by the live SaaS
timings in Section~\ref{sec:livesaas}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    Policy Violation (live)     & \checkmark & --         & --         \\
...
Replay validates semantic alignment between AIT's detection categories
and independently reported real-world violations, complementing the
direct sandbox evidence from Section~\ref{sec:livesaas}.
...
ground-truth API logs. This experiment provides \emph{plausibility
evidence} that AIT's detection categories map to real incident classes,
not empirical precision/recall on real platform data.
...
provides direct evidence that the same detection logic surfaces real
policy divergence on live vendor APIs without any modification.
...
two live sandbox accounts (GitHub and Notion), a read-only GitHub
smoke probe, and replayed public incidents. The framework has not been
...
policies; the live suite covers two sandbox accounts. Together they
...
ground-truth dataset of real integration violations. The
$+30$ policy-violation addend in the live runner is a design choice,
...
\emph{instrumentation-free} taint markers for C2. A new live SaaS
evaluation pipeline extends validation beyond mocks: sandbox runs on
GitHub and Notion confirm that the same detection logic surfaces real
policy divergence on live vendor REST APIs, with the GitHub sandbox
producing a policy violation finding (risk~30, High) and the Notion
sandbox correctly returning no findings on a compliant integration.
